...
function Header() {
  return (
    <header className="max-w-3xl mx-auto text-center mt-8 mb-6">
      <h1 className="text-3xl font-extrabold">SPC식품안전문화 MBTI 테스트</h1>
      <p className="text-sm opacity-70 mt-2">
        본 테스트는 식품안전문화 참여를 위해 기존 MBTI를 응용한 내용이며, 테스트 결과는 익명으로 이
        브라우저에만 저장됩니다.
      </p>
    </header>
  );
...
function compute(answers) {
  ...
  for (const q of QUESTIONS.items) {
    ...
    const left = QUESTIONS.axes.find((ax) => ax.key === q.axis).left;
      ...
      {QUESTIONS.axes.map((ax) => (
        <div key={ax.key} className="p-4 rounded-2xl border bg-white">
          ...
          <div className="w-full bg-gray-200 h-3 rounded-full overflow-hidden">
            <div
              className="h-3"
              style={{ width: `${Math.round((normalized[ax.key] ?? 0.5) * 100)}%`, background: BRAND.primary }}
            />
          </div>
        </div>
      ))}
    ...
    <div className="flex items-center gap-4 p-3 rounded-xl border">
      <div className="hidden sm:block">
        {qr ? (
          ...
          <div className="w-24 h-24 grid place-items-center text-xs opacity-60">배포 후 QR</div>
        )}
      </div>
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-2 flex gap-2">
          ...
          {qr && (
            <a className="px-3 py-1 rounded-lg border" href={qr} download>
              QR 다운로드
            </a>
          )}
        </div>
        ...
        {started && !show && (
          <div className="space-y-4">
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <button
                disabled={answered < QUESTIONS.items.length}
        ...
        {started && show && (
          <div className="space-y-6">
                ...
                return (
                  <>
                    ...
                    <div className="space-y-2 mt-4">
                      <h3 className="font-semibold">성격 요약</h3>
            ...
            <div className="rounded-2xl border bg-white/90 p-4 flex flex-wrap gap-2">
              <ShareLinkQR />
              <button className="px-3 py-2 rounded-lg border" onClick={() => window.print()}>
                인쇄하기
              </button>
            </div>
          </div>
